...
define( 'WC_MIN_MAX_QUANTITIES', '2.3.16' );
	...
	public function check_cart_items() {
		...
		if ( $apply_cart_rules ) {
			...
			if ( $this->minimum_order_value && $total_cost && $total_cost < $this->minimum_order_value ) {
				...
				$this->add_error( sprintf( __( 'The minimum allowed order value is %s - please add more items to your cart', 'woocommerce-min-max-quantities' ), wc_price( $this->minimum_order_value ) ) . $excludes );
