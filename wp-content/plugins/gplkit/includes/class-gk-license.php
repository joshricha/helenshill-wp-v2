...
if ( get_option( 'gplkit_plugin_manager_activated' ) != 'Activated' ) {
    add_action( 'admin_notices', 'GplKit_Plugin_Manager::gk_inactive_notice' );
}

class GplKit_Plugin_Manager {

	/**
	 * Self Upgrade Values
	 */
	// Base URL to the remote upgrade API Manager server. If not set then the Author URI is used.
	public $upgrade_url = 'http://www.gplkit.com/';
	 ...
	 */
	public $gplkit_plugin_manager_version_name = 'plugin_gplkit_plugin_manager_version';
	 ...
	 * @var string
	 */
	public $text_domain = 'gplkit-plugin-manager';
	...
	public function __construct() {
		...
		if ( is_admin() ) {
			 ...
			 */
			$this->ame_software_product_id = 'GplKit Plugin Manager';

			/**
			 * Set all data defaults here
			 */
			$this->ame_data_key 				= 'gplkit_plugin_manager';
			$this->ame_api_key 					= 'api_key';
			$this->ame_activation_email 		= 'activation_email';
			$this->ame_product_id_key 			= 'gplkit_plugin_manager_product_id';
			$this->ame_instance_key 			= 'gplkit_plugin_manager_instance';
			$this->ame_deactivate_checkbox_key 	= 'gplkit_plugin_manager_deactivate_checkbox';
			$this->ame_activated_key 			= 'gplkit_plugin_manager_activated';

			/**
			 * Set all admin menu data
			 */
			$this->ame_deactivate_checkbox 			= 'am_deactivate_example_checkbox';
			$this->ame_activation_tab_key 			= 'gplkit_settings_page';
			$this->ame_deactivation_tab_key 		= 'gplkit_plugin_manager_deactivation';
			$this->ame_settings_menu_title			= 'GPL Kit Dashboard';
			$this->ame_settings_title 				= 'GPL Kit Dashboard';
			$this->ame_menu_tab_activation_title 	= __( 'License Activation', 'gplkit-plugin-manager' );
			$this->ame_menu_tab_deactivation_title 	= __( 'License Deactivation', 'gplkit-plugin-manager' );

			/**
			 * Set all software update data here
			 */
			$this->ame_options 				= get_option( $this->ame_data_key );
			$this->ame_plugin_name 			= 'gplkit/gplkit.php'; // same as plugin slug. if a theme use a theme name like 'twentyeleven'
			$this->ame_product_id 			= get_option( $this->ame_product_id_key ); // Software Title
			$this->ame_renew_license_url 	= 'https://www.gplkit.com/my-account'; // URL to renew a license. Trailing slash in the upgrade_url is required.
	 ...
	 * API Key Class.
	 *
	 * @return GplKit_Plugin_Manager_Key
	 */
	public function key() {
		return GplKit_Plugin_Manager_Key::instance();
	}

	/**
	 * Update Check Class.
	 *
	 * @return GplKit_Plugin_Manager_Update_API_Check
	 */
	public function update_check( $upgrade_url, $plugin_name, $product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra = '' ) {
		...
		return GplKit_Plugin_Manager_Update_API_Check::instance( $upgrade_url, $plugin_name, $product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra );
	}
	...
	public function activation() {
		...
		$curr_ver = get_option( $this->gplkit_plugin_manager_version_name );

		// checks if the current plugin version is lower than the version being installed
		if ( version_compare( $this->version, $curr_ver, '>' ) ) {
			// update the version
			update_option( $this->gplkit_plugin_manager_version_name, $this->version );
		}
     ...
     */
	public static function gk_inactive_notice() { ?>
		<?php if ( ! current_user_can( 'manage_options' ) ) return; ?>
		<?php if ( isset( $_GET['page'] ) && 'gplkit_plugin_manager_dashboard' == $_GET['page'] ) return; ?>
		<div id="message" class="error">
			<p><?php printf( __( 'GPL Kit Plugin Manager has not been activated, so the plugin is inactive! %sClick here%s to activate the license key and the plugin.', 'gplkit-plugin-manager' ), '<a href="' . esc_url( admin_url( 'admin.php?page=gplkit_settings_page' ) ) . '">', '</a>' ); ?></p>
		</div>
		<?php
	}
	...
	public function check_external_blocking() {
		...
		if( defined( 'WP_HTTP_BLOCK_EXTERNAL' ) && WP_HTTP_BLOCK_EXTERNAL === true ) {
			...
			if( ! defined( 'WP_ACCESSIBLE_HOSTS' ) || stristr( WP_ACCESSIBLE_HOSTS, $host ) === false ) {
				?>
				<div class="error">
					<p><?php printf( __( '<b>Warning!</b> You\'re blocking external requests which means you won\'t be able to get %s updates. Please add %s to %s.', 'gplkit-plugin-manager' ), $this->ame_software_product_id, '<strong>' . $host . '</strong>', '<code>WP_ACCESSIBLE_HOSTS</code>'); ?></p>
				</div>
				<?php
			}
...
function GKL() {
    return GplKit_Plugin_Manager::instance();
}
