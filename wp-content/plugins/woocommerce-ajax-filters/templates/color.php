...
if ( @ $terms ) {
    foreach ( $terms as $term ) {
        ...
        ?>
        <li class="berocket_term_parent_<?php echo @$term->parent; ?> <?php if ( $is_child_parent ) echo 'R__class__R '; ?><?php if( @ $hide_o_value && isset($term->count) && $term->count == 0 && ( !$is_child_parent || !$is_first ) ) { echo 'berocket_hide_o_value '; $hiden_value = true; }  if( @ $hide_sel_value && @ br_is_term_selected( $term, true, $is_child_parent_or, $child_parent_depth ) != '' ) { echo 'berocket_hide_sel_value'; $hiden_value = true; } ?> berocket_checkbox_color<?php echo ( ( ( @ $use_value_with_color ) ? ' berocket_color_with_value' : ' berocket_color_without_value' ) ) ?>">
            <span>
                ...
                <label data-for='checkbox_<?php echo @ str_replace ( '*' , '-' , $term->term_id), @ str_replace ( '*' , '-' , $term->taxonomy) ?>' 
                    ...
                    <span class="berocket_color_span_block <?php if( ! @ $meta_after ) echo 'berocket_aapf_count'; ?>" style="<?php echo $meta_color; ?>"><?php if( $type == 'color' ) echo '<span class="berocket_color_span_absolute"><span>'; ?><?php echo $meta_class; ?><?php if( $type == 'color' ) echo '</span></span>'; ?></span>
