	...
	*/
	public function wpml() {
		if ( isset($this->options['wpml_string_translation']) && function_exists( 'icl_register_string' ) ) {
			icl_register_string('WP Menu Cart', 'item text', '');
			icl_register_string('WP Menu Cart', 'items text', '');
	...
	public function wpmenucart_menu_item() {
		...
		if ( isset($this->options['wpml_string_translation']) && function_exists( 'icl_t' ) ) {
			...
			$cart_contents = $item_data['cart_contents_count'] .' '. ( $item_data['cart_contents_count'] == 1 ?  icl_t('WP Menu Cart', 'item text', '') :  icl_t('WP Menu Cart', 'items text', '') );
		} else {
			//use regular WP i18n
			$viewing_cart = __('View your shopping cart', 'wpmenucart');
			$start_shopping = __('Start shopping', 'wpmenucart');
			$cart_contents = sprintf(_n('%d', '%d', $item_data['cart_contents_count'], 'wpmenucart'), $item_data['cart_contents_count']);
		}
